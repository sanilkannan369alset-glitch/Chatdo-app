        ...
        <div className="authForm">
          ...
          {mode === "forgot" ? (
            ...
            <>
              ...
              {mode === "signup" && <input className="input" placeholder="Mobile number (optional)" value={phone} onChange={e => setPhone(e.target.value)} />}
              ...
              <button className="secondary" onClick={google} disabled={busy}>Continue with Google</button>

              <div style={{textAlign:"center",color:"#9a7a65",fontSize:12}}>or mobile OTP</div>
  ...
  return (
    <div className="page">
      <div className="pageHead">
        ...
        {menu && <div className="contextMenu" style={{right:20,top:65}}><button onClick={clearHistory}>Clear all search history</button></div>}
      </div>
        ...
        <div>
          ...
          {history.map((x,i) => (
            <div className="chatRow" key={`${x}-${i}`}>
              ...
              <button className="secondary" onClick={() => removeHistory(x)}>×</button>
            </div>
          ))}
        </div>
      )}
...
function Chat({ me, other, onBack }) {
  ...
  useEffect(() => {
    ...
    setDoc(cRef,{members:[me.uid,other.uid].sort(),updatedAt:serverTimestamp()},{merge:true}).catch(()=>{});
    const q = query(collection(db,"chats",id,"messages"),orderBy("createdAt","asc"));
    return onSnapshot(q, snap => {
      const list=snap.docs.map(d=>({id:d.id,...d.data()}));
      setMessages(list);
      list.filter(m=>m.senderId!==me.uid).forEach(m=>{
        updateDoc(doc(db,"chats",id,"messages",m.id),{
          deliveredTo:arrayUnion(me.uid),seenBy:arrayUnion(me.uid)
        }).catch(()=>{});
      });
    });
  },[id,me.uid,other.uid]);
  ...
  const send = async () => {
    ...
    if(!friends && sentCount>=5) return alert("Before becoming friends, you can send only 5 introductory messages.");
  ...
  const editMessage = m => {
    ...
    if(Date.now()-ts(m.createdAt)>2*60*60*1000) return alert("Messages can only be edited within 2 hours.");
    setText(m.text||"");setEditing(m);setMenu(null);
  };

  const delForMe = async m => {
    await updateDoc(doc(db,"chats",id,"messages",m.id),{deletedFor:arrayUnion(me.uid)});setMenu(null);
  };

  const delEveryone = async m => {
    if(m.senderId!==me.uid) return;
    await updateDoc(doc(db,"chats",id,"messages",m.id),{text:"This message was deleted",deletedForEveryone:true});setMenu(null);
  };

  const react = async (m,emoji) => {
    const r=m.reactions||{};
    const users=r[emoji]||[];
    const next=users.includes(me.uid)?users.filter(x=>x!==me.uid):[...users,me.uid];
    await updateDoc(doc(db,"chats",id,"messages",m.id),{[`reactions.${emoji}`]:next});
  };
    ...
    try{
      ...
      rec.onstop=async()=>{
        ...
        try{
          ...
          const url=await uploadFile(me.uid,new File([blob],"voice.webm",{type:"audio/webm"}),"chat-media");
          await addDoc(collection(db,"chats",id,"messages"),{senderId:me.uid,receiverId:other.uid,type:"audio",audioURL:url,createdAt:serverTimestamp(),deliveredTo:[me.uid],seenBy:[me.uid]});
          await setDoc(doc(db,"chats",id),{members:[me.uid,other.uid].sort(),lastMessage:"🎤 Voice message",updatedAt:serverTimestamp()},{merge:true});
        }catch{}
        setRecording(false);
      };
      mediaRecorder.current=rec;rec.start();setRecording(true);
  ...
  const sendMedia=async file=>{
    ...
    try{
      ...
      await setDoc(doc(db,"chats",id),{members:[me.uid,other.uid].sort(),lastMessage:type==="image"?"📷 Photo":"🎥 Video",updatedAt:serverTimestamp()},{merge:true});
    }catch{}
  };

  return (
    <div className="page chatPage" onClick={()=>menu&&setMenu(null)}>
      <div className="chatHeader">
        ...
        <button className="iconBtn" onClick={()=>alert("Start audio call from Calls tab.")}>☎</button>
        <button className="iconBtn" onClick={()=>alert("Start video call from Calls tab.")}>▣</button>
      </div>
        ...
        {messages.filter(m=>!(m.deletedFor||[]).includes(me.uid)).map(m=>{
          ...
          return <div className={`messageLine ${mine?"mine":""}`} key={m.id}>
            <div className="bubble" onContextMenu={e=>{e.preventDefault();setMenu({m,x:e.clientX,y:e.clientY})}}>
              ...
              {m.type==="audio" && <audio src={m.audioURL} controls/>}
              {m.type==="text" && <div className="bubbleText">{m.text}</div>}
              {m.reactions && <div style={{marginTop:5}}>{Object.entries(m.reactions).filter(([,v])=>v?.length).map(([e,v])=><span key={e} style={{marginRight:4}}>{e} {v.length}</span>)}</div>}
      ...
      {reply && <div className="replyComposer">↪ Replying to: {reply.text||"media"} <button className="linkBtn" onClick={()=>setReply(null)}>×</button></div>}
      ...
      <div className="composer">
        <label className="iconBtn">＋<input hidden type="file" accept="image/*,video/*" onChange={e=>sendMedia(e.target.files?.[0])}/></label>
        <button className={`iconBtn ${recording?"danger":""}`} onClick={startRecording}>{recording?"■":"🎤"}</button>
        <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Message..." onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send()}}}/>
        <button className="sendBtn" onClick={send}>➤</button>
      </div>

      {menu && <div className="contextMenu" style={{left:menu.x,top:menu.y}} onClick={e=>e.stopPropagation()}>
        <button onClick={()=>{setReply(menu.m);setMenu(null)}}>↩ Reply</button>
        <div className="emojiBar" style={{padding:8}}>{["❤️","👍","😂","😮","😢","🔥"].map(e=><button key={e} onClick={()=>{react(menu.m,e);setMenu(null)}}>{e}</button>)}</div>
        {menu.m.senderId===me.uid && <button onClick={()=>editMessage(menu.m)}>✏ Edit</button>}
        <button onClick={()=>delForMe(menu.m)}>Delete for me</button>
        {menu.m.senderId===me.uid && <button className="dangerItem" onClick={()=>delEveryone(menu.m)}>Delete for everyone</button>}
      </div>}
    </div>
  );
}
...
function Stories({ me, allUsers, openProfile }) {
  ...
  useEffect(()=>{
    ...
    return onSnapshot(q,snap=>{
      ...
      setStories(snap.docs.map(d=>({id:d.id,...d.data()})).filter(s=>ts(s.createdAt)>=cutoff && (!s.expiresAt || ts(s.expiresAt)>Date.now())));
    });
  },[]);

  const mine=stories.filter(s=>s.uid===me.uid);
  const others=stories.filter(s=>s.uid!==me.uid);
  const userMap=Object.fromEntries(allUsers.map(x=>[x.uid,x]));
  const ordered=[...others].sort((a,b)=>ts(b.createdAt)-ts(a.createdAt));

  const post=async()=>{
    if(!file)return alert("Choose a photo/video first.");
    try{
      const url=await uploadFile(me.uid,file,"stories");
      const type=file.type.startsWith("video")?"video":"image";
      await addDoc(collection(db,"stories"),{
        uid:me.uid,url,type,text,textColor,textSize,sticker,stickerSize,
        createdAt:serverTimestamp(),expiresAt:new Date(Date.now()+24*60*60*1000)
      });
      setEditor(false);setFile(null);setText("");
    }catch{}
  };
  ...
  return (
    <div className="page">
      <div className="pageHead"><div><div className="pageTitle">Stories</div><div className="subtle">24-hour stories • 30 sec+ video supported</div></div><button className="primary" onClick={()=>setEditor(true)}>＋ My Story</button></div>

      <div className="storiesStrip">
        <div className="storyItem" onClick={()=>mine[0]&&setViewer(mine[0])}>
          <div className="storyCircle">{mine[0]?.url?<img src={mine[0].url} alt=""/>:avatar(me)}<span className="storyAdd">＋</span></div><div>My Story</div>
        </div>
        {ordered.map(s=>{
          const u=userMap[s.uid]||{uid:s.uid};
          return <div className="storyItem" key={s.id} onClick={()=>setViewer(s)}>
            <div className="storyCircle">{s.url?<img src={s.url} alt=""/>:avatar(u)}</div><div>{nameOf(u)}</div>
          </div>
        })}
      </div>
      ...
      <div className="storyGrid">
        {[...mine,...ordered].map(s=>{
          const u=s.uid===me.uid?me:(userMap[s.uid]||{uid:s.uid});
          return <div className="storyCard" key={s.id}>
            <div className="storyMedia" onClick={()=>setViewer(s)}>
              {s.type==="video"?<video src={s.url}/>:<img src={s.url} alt=""/>}
              {s.text&&<div className="storyOverlayText" style={{color:s.textColor,fontSize:s.textSize}}>{s.text}</div>}
              {s.sticker&&<div style={{position:"absolute",right:10,bottom:10,fontSize:s.stickerSize}}>{s.sticker}</div>}
            </div>
            <div className="storyInfo"><b>{nameOf(u)}</b><div className="subtle">{dateText(s.createdAt)}</div>
              <div className="storyActions"><button className="secondary" onClick={()=>openProfile(u)}>Profile</button>{s.uid===me.uid&&<button className="danger" onClick={()=>deleteStory(s.id)}>Delete</button>}</div>
            </div>
          </div>
        })}
      </div>

      {editor&&<div className="modalBack"><div className="modal">
        <div className="modalHead"><div className="modalTitle">Story editor</div><button className="iconBtn" onClick={()=>setEditor(false)}>×</button></div>
        <div className="formGrid">
          <input className="input" type="file" accept="image/*,video/*" onChange={e=>setFile(e.target.files?.[0])}/>
          {file&&<div className="storyMedia">{file.type.startsWith("video")?<video src={URL.createObjectURL(file)} controls/>:<img src={URL.createObjectURL(file)} alt="preview"/>}{text&&<div className="storyOverlayText" style={{color:textColor,fontSize:textSize}}>{text}</div>}{sticker&&<div style={{position:"absolute",right:10,bottom:10,fontSize:stickerSize}}>{sticker}</div>}</div>}
          <input className="input" placeholder="Add text" value={text} onChange={e=>setText(e.target.value)}/>
          <div className="twoCol"><label className="fieldLabel">Text colour<input type="color" value={textColor} onChange={e=>setTextColor(e.target.value)}/></label><label className="fieldLabel">Text size<input type="range" min="16" max="72" value={textSize} onChange={e=>setTextSize(Number(e.target.value))}/></label></div>
          <div className="emojiBar">{["😊","😂","❤️","🔥","😍","😎","👍","🎉","⭐"].map(e=><button key={e} onClick={()=>setSticker(e)}>{e}</button>)}</div>
          <label className="fieldLabel">Sticker size<input type="range" min="25" max="110" value={stickerSize} onChange={e=>setStickerSize(Number(e.target.value))}/></label>
          <div className="notice">Preview is shown above before posting. Storage must be enabled in Firebase for media upload.</div>
          <button className="primary" onClick={post}>Post story</button>
        </div>
      </div></div>}

      {viewer&&<div className="modalBack" onClick={()=>setViewer(null)}><div className="modal" onClick={e=>e.stopPropagation()}>
        <div className="modalHead"><div className="modalTitle">Story</div><button className="iconBtn" onClick={()=>setViewer(null)}>×</button></div>
        <div className="storyMedia" style={{height:"65vh"}}>{viewer.type==="video"?<video src={viewer.url} controls autoPlay/>:<img src={viewer.url} alt=""/>}{viewer.text&&<div className="storyOverlayText" style={{color:viewer.textColor,fontSize:viewer.textSize}}>{viewer.text}</div>}{viewer.sticker&&<div style={{position:"absolute",right:18,bottom:18,fontSize:viewer.stickerSize}}>{viewer.sticker}</div>}</div>
      </div></div>}
    </div>
  );
}
...
function Settings({ me, refresh }) {
  ...
  const save=async()=>{
    setBusy(true);
    try{
      let photoURL=me.photoURL||"",coverURL=me.coverURL||"";
      if(photo)photoURL=await uploadFile(me.uid,photo,"profiles");
      if(cover)coverURL=await uploadFile(me.uid,cover,"covers");
      const lower=normalize(username);
      if(lower && !/^[a-z0-9._]{3,24}$/.test(lower)) throw new Error("Username: 3-24 letters, numbers, dot or underscore.");
      const fields={username:lower,displayName,bio,hobbies,phoneNumber:phoneOnly(phone),photoURL,coverURL};
      await saveUserFields(me.uid,fields);
      await updateProfile(auth.currentUser,{displayName,photoURL});
      alert("Profile saved.");
    }catch(e){alert(e?.message||"Could not save profile.");}
    finally{setBusy(false)}
  };

  return <div className="page">
    <div className="pageHead"><div><div className="pageTitle">Profile & Settings</div><div className="subtle">Edit your Chatdo profile</div></div><button className="primary" onClick={save} disabled={busy}>Save</button></div>
    <div className="profileCard">
      <div className="cover">{me.coverURL&&<img src={me.coverURL} alt="cover"/>}</div>
      <div className="profileBody"><div className="profileAvatar">{avatar(me)}</div></div>
      <div className="profileFields">
        <label className="fieldLabel">Profile picture<input className="input" type="file" accept="image/*" onChange={e=>setPhoto(e.target.files?.[0])}/></label>
        <label className="fieldLabel">Cover image<input className="input" type="file" accept="image/*" onChange={e=>setCover(e.target.files?.[0])}/></label>
        <label className="fieldLabel">Name<input className="input" value={displayName} onChange={e=>setDisplayName(e.target.value)}/></label>
        <label className="fieldLabel">Username<input className="input" value={username} onChange={e=>setUsername(e.target.value)} placeholder="yourusername"/></label>
        <label className="fieldLabel">Bio<textarea value={bio} onChange={e=>setBio(e.target.value)}/></label>
        <label className="fieldLabel">Hobbies<input className="input" value={hobbies} onChange={e=>setHobbies(e.target.value)}/></label>
        <label className="fieldLabel">Mobile number<input className="input" value={phone} onChange={e=>setPhone(e.target.value)}/></label>
        <label className="fieldLabel">Email<input className="input" value={me.email||""} disabled/></label>
      </div>
    </div>
    <div className="settingsList">
      <div className="settingRow"><div><b>Account</b><div className="subtle">Logout and account switching</div></div><button className="secondary" onClick={()=>signOut(auth)}>Logout</button></div>
      <div className="settingRow"><div><b>Privacy & Safety</b><div className="subtle">Blocked users, reports and notifications are stored in Firebase.</div></div><span>🔒</span></div>
      <div className="settingRow"><div><b>Chatdo Legal</b><div className="subtle">Privacy Policy • Terms • Contact • Safety</div></div><span>ℹ️</span></div>
    </div>
  </div>;
}
...
function App() {
  ...
  useEffect(()=>{
    if(!fbUser)return;
    const before=()=>saveUserFields(fbUser.uid,{online:false,lastSeen:serverTimestamp()}).catch(()=>{});
    window.addEventListener("beforeunload",before);
    return ()=>window.removeEventListener("beforeunload",before);
  },[fbUser]);

  const me=useUserDoc(fbUser?.uid);
  const allUsers=useAllUsers(fbUser?.uid);
  ...
  return <div className="app">
    ...
    <div className="topbar">
      <div className="brand" onClick={()=>setLegal(true)}><div className="brandMark">C</div><span>Chatdo</span></div>
      <div className="topActions">
        <button className="iconBtn" onClick={()=>openProfile(me)}>👤</button>
      </div>
    </div>

    <div className="mainWrap">
      <div className="nav">
        {TABS.map(t=><button key={t} className={tab===t?"active":""} onClick={()=>{setTab(t);setChatUser(null)}}>{t==="chats"?"💬 Chat":t==="stories"?"⭕ Story":t==="calls"?"☎ Call":"👥 People"}</button>)}
        <button onClick={()=>setTab("settings")}>⚙ Settings</button>
      </div>

      {chatUser ? <Chat me={me} other={chatUser} onBack={()=>setChatUser(null)}/> :
       tab==="chats" ? <Chats me={me} allUsers={allUsers} openChat={openChat}/> :
       tab==="stories" ? <Stories me={me} allUsers={allUsers} openProfile={openProfile}/> :
       tab==="calls" ? <Calls me={me} allUsers={allUsers} openProfile={openProfile}/> :
       tab==="people" ? <People me={me} allUsers={allUsers} openChat={openChat} openProfile={openProfile}/> :
       <Settings me={me}/>}
    </div>

    {profileUser&&<ProfileModal me={me} user={profileUser} onClose={()=>setProfileUser(null)} openChat={u=>{setProfileUser(null);openChat(u)}}/>}
    {legal&&<div onClick={()=>setLegal(false)}><Legal/></div>}
  </div>;
}
