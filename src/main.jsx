...
function Styles() {
  return <style>{`
    *{box-sizing:border-box}
    ...
    .editIcon{position:absolute;right:8px;bottom:8px;width:34px;height:34px;border-radius:50%;border:2px solid #fffaf3;background:#70462f;color:#fff;display:grid;place-items:center;cursor:pointer;box-shadow:0 5px 15px #0003}
    .coverEdit{position:absolute;right:10px;top:10px}
...
function Settings({ me, refresh }) {
  ...
  return <div className="page"><div className="pageHead"><div><div className="pageTitle">Profile & Settings</div><div className="subtle">Edit your Chatdo profile</div></div><button className="primary" onClick={save} disabled={busy}>Save</button></div><div className="profileCard"><div className="cover">{me.coverURL&&<img src={me.coverURL} alt="cover"/>}<label className="editIcon coverEdit" title="Edit cover photo">✏️<input hidden type="file" accept="image/*" onChange={e=>setCover(e.target.files?.[0]||null)}/></label></div><div className="profileBody"><div className="profileAvatar" style={{position:"relative"}}>{avatar(me)}<label className="editIcon" title="Edit profile photo">✏️<input hidden type="file" accept="image/*" onChange={e=>setPhoto(e.target.files?.[0]||null)}/></label></div></div><div className="profileFields"><label className="fieldLabel">Name<input className="input" value={displayName} onChange={e=>setDisplayName(e.target.value)}/></label><label className="fieldLabel">Username<input className="input" value={username} onChange={e=>setUsername(e.target.value)} placeholder="yourusername"/></label><label className="fieldLabel">Bio<textarea value={bio} onChange={e=>setBio(e.target.value)}/></label><label className="fieldLabel">Hobbies<input className="input" value={hobbies} onChange={e=>setHobbies(e.target.value)}/></label><label className="fieldLabel">Mobile number<input className="input" value={phone} onChange={e=>setPhone(e.target.value)}/></label><label className="fieldLabel">Email<input className="input" value={me.email||""} disabled/></label></div></div><div className="settingsList"><div className="settingRow"><div><b>Account</b><div className="subtle">Logout and account switching</div></div><button className="secondary" onClick={()=>signOut(auth)}>Logout</button></div><div className="settingRow"><div><b>Privacy & Safety</b><div className="subtle">Blocked users, reports and notifications are stored in Firebase.</div></div><span>🔒</span></div><div className="settingRow"><div><b>Chatdo Legal</b><div className="subtle">Privacy Policy • Terms • Contact • Safety</div></div><span>ℹ️</span></div></div></div>;
}
